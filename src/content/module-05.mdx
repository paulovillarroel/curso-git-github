...
Después de hacer merge, la rama ya cumplió su propósito. Puedes borrarla para mantener las cosas ordenadas.

```bash
# Borrar la rama (solo funciona si ya fue mergeada)
git branch -d feature/saludo
# Deleted branch feature/saludo (was e5f6a7b).
```

La `-d` es de "delete" (borrar). Si la rama no ha sido mergeada, Git te avisará para que no pierdas trabajo accidentalmente.
...
<CommandTable commands={[
  ...
  { command: "git branch -d nombre-rama", description: "Borrar una rama ya mergeada" },
]} />
