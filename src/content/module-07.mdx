...
### Mantener tu rama actualizada con rebase
...
**¿Por qué rebase y no merge?** Si haces `git merge main` dentro de tu rama, funciona, pero crea commits de merge extras que ensucian la historia y hacen tu PR más difícil de revisar. Con rebase, tu rama queda como si la hubieras creado recién desde el `main` más actualizado: limpia y fácil de revisar.

**Atajo: `git pull --rebase`**
...
También existe `git pull --merge` (que es lo que `git pull` hace por defecto): trae los cambios remotos y crea un commit de merge para combinarlos. La diferencia es la misma que ya conoces — `--rebase` mantiene la historia lineal, `--merge` preserva las bifurcaciones.
